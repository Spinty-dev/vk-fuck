...
private class AndroidProcessHost : ProcessHost {
    ...
    override fun start(binary: String, args: List<String>, env: Map<String, String>) {
        ...
        pump(proc.inputStream.bufferedReader(), stdout, "stdout")
        pump(proc.errorStream.bufferedReader(), stderr, "stderr")

        scope.launch {
            val code = runCatching { proc.waitFor() }.getOrNull() ?: -1
            exits.emit(ProcessHost.Exit(code = code, killed = wasKilled))
        }
    }
        ...
        scope.launch {
            try {
                while (true) {
                    ...
                    into.emit(ProcessHost.Line(tag, line))
                }
            } catch (_: Throwable) {
            } finally {
...
actual fun createProcessHost(): ProcessHost = AndroidProcessHost()
