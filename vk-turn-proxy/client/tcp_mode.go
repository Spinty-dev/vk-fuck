...
// waitAssignableTCPSmuxSession waits until the pool exposes a usable smux session
// or ctx is canceled. Matches Android vkturn pre-warm probing: callers may dial
// -listen TCP before outbound TURN/DTLS sessions finish handshaking; we hold the
// connection instead of rejecting (previously Listen followed pool.count()>0).
func waitAssignableTCPSmuxSession(ctx context.Context, pool *tcpSessionPool) *smux.Session {
	tick := time.NewTicker(50 * time.Millisecond)
	defer tick.Stop()
	for {
		if s := pool.pick(); s != nil && !s.IsClosed() {
			return s
		}
		select {
		case <-ctx.Done():
...
func runTCPMode(ctx context.Context, turnConfig *turnParams, peer *net.UDPAddr, listenAddr string, sessionCount int) {
	...
	log.Printf("TCP mode: waking %d outbound session worker(s); local listen binds immediately", sessionCount)
	listener, err := net.Listen("tcp", listenAddr)
	if err != nil {
		log.Panicf("TCP listen: %s", err)
	}
	context.AfterFunc(ctx, func() { _ = listener.Close() })
	log.Printf("TCP mode: listening on %s (round-robin across %d sessions; peers may dial before relay is warm)", listenAddr, sessionCount)
	...
	for {
		...
		go func(localConn net.Conn) {
			defer accepted.Done()
			smuxSession := waitAssignableTCPSmuxSession(ctx, pool)
			if smuxSession == nil {
				log.Printf("TCP accept cancelled before smux pool became ready")
				_ = localConn.Close()
				return
			}
